...
const submitOrder = async () => {
  ...
  try {
    var result = await fetch(`${serverLocalURL}/orders`, {
      method: "POST",
...
<template>
  ...
  <div class="container mx-auto p-6 min-h-screen font-sans max-w-6xl">

    <!-- HEADER -->
    <header class=" flex flex-col md:flex-row justify-between items-center rounded-xl">
    ...
    <div v-if="!showCart">
      ...
      <!-- CONTROLS BAR -->
      <div class="controlBar rounded-xl ">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div class="flex flex-col">
            ...
            <select v-model="sortAttribute"
              class="border-4 border-black p-3 rounded-lg font-bold bg-white cursor-pointer">
          ...
          <div class="flex flex-col">
            ...
            <div class="flex bg-white border-4 border-black rounded-lg overflow-hidden h-full">
              <label
                class="flex-1 flex items-center justify-center cursor-pointer hover:bg-gray-100 border-r-4 border-black font-bold">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <div v-for="lesson in filteredLessons" :key="lesson.id"
          class="containerLesson p-6 rounded-xl hover:-translate-y-2 transition-transform duration-200 flex flex-col relative overflow-hidden group">

          <!-- Decorative Circle Background -->
          <div
            class="absolute -top-10 -right-10 w-32 h-32 bg-yellow-200 rounded-full border-4 border-black z-0 group-hover:bg-yellow-300 transition-colors">
          </div>

          <div class="relative z-10 flex justify-between items-start mb-4">
            <h2 class="text-3xl font-black text-black transform -rotate-1">{{ lesson.subject }}</h2>
            <i :class="lesson.icon"
              class="text-3xl text-black bg-white p-2 border-2 border-black rounded-lg shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]"></i>
          </div>
          ...
          <div class="relative z-10 space-y-3 mb-6 font-bold text-gray-700">
            ...
            <div class="flex items-center mt-2">
              ...
              <span
                class="px-3 py-1 rounded-full border-2 border-black font-black text-sm uppercase shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
    ...
    <div v-else class="flex flex-col lg:flex-row gap-8">
      <div class="flex-1 bg-white p-8 rounded-xl border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
        ...
        <div v-for="(item, index) in cart" :key="index"
          class="flex justify-between items-center mb-4 p-4 bg-blue-50 border-2 border-black rounded-lg">
          <div>
          ...
          <div class="flex items-center gap-4">
            ...
            <div class="flex items-center gap-3 mr-6">
              <button @click="decreaseCartQuantity(item, index)"
                class="bg-gray-200 text-gray-700 w-8 h-8 rounded-full hover:bg-gray-300 font-bold">
                -
              </button>
              <span class="font-bold text-lg">{{ item.quantity }}</span>
              <button @click="increaseCartQuantity(item)" :disabled="item.lesson.spaces === 0"
      ...
      <div
        class="w-full md:w-1/3 bg-[#98FB98] p-6 rounded-xl border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] h-fit">
        ...
        <div class="mb-4">
          ...
          <input v-model="checkoutForm.name" type="text"
            class="w-full border-4 border-black p-3 rounded-lg focus:outline-none focus:ring-4 focus:ring-white">
        </div>
        <div class="mb-6">
          <label class="block font-bold mb-2">Phone Number:</label>
          <input v-model="checkoutForm.phone" type="text"
            class="w-full border-4 border-black p-3 rounded-lg focus:outline-none focus:ring-4 focus:ring-white">
        </div>
        <button @click="submitOrder" :disabled="!isFormValid || cart.length === 0"
